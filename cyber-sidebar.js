...
class CyberSidebar {
  ...
  init() {
    ...
    document.querySelectorAll('.cyber-menu-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        // 如果是课程，直接跳转到课程页面
        if (section === 'courses') {
          window.location.href = 'courses/index.html'; // 跳转到课程主页
          return;
        }
  ...
  files = {
    novels: [
      ...
      { name: '数字幽灵', file: 'book/数字幽灵.txt' },
      { name: '霓虹之夜', file: 'book/霓虹之夜.txt' }
    ],
    images: [
      { name: '卡提西亚', file: 'Picture/卡提西亚.jpg' },
      { name: '赛琳娜', file: 'Picture/赛琳娜.jpg' },
      { name: '钢琴赛赛', file: 'Picture/钢琴赛赛.jpg' },
      { name: 'YOCIM', file: 'Picture/YOCIM.png' }
    ],
    // 改为HTML课程页面
    courses: [
      { 
        name: 'HTML5入门', 
        file: 'courses/html5-basics.html',
        description: '学习HTML5基础语法和标签使用'
      },
      { 
        name: 'CSS3进阶', 
        file: 'courses/css3-advanced.html',
        description: '掌握CSS3高级特性和动画效果'
      },
      { 
        name: 'JavaScript实战', 
        file: 'courses/js-practical.html',
