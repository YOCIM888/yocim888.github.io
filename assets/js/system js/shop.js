...
function updateCyberTime() {
    const now = new Date();
    const cyberDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), now.getMinutes(), now.getSeconds());
...
function purchaseProduct(productId) {
    ...
    if (balance >= product.price) {
        ...
        // 显示发货通知
        showNotification(`订单确认: ${product.name} 将于3033年6月6日发货至指定领取传送点`);
