...
function getLevelParams(level) {
    const baseMoves = config.moves;
    const baseWinScore = config.winScore;
    const multiplier = Math.pow(1.5, level - 1);
    return {
        moves: Math.floor(baseMoves * multiplier),
        winScore: Math.floor(baseWinScore * multiplier)
    };
}
...
function initGame(level = 1) {
    ...
    if (level === 1) {
        ...
        gameStatusElement.textContent = `第 ${level} 关开始！目标分数: ${levelParams.winScore}`;
    }
...
function processMatches(matches) {
    ...
    gameState.levelScore += matchScore;
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (newMatches.length > 0) {
                ...
                // 检查是否完成当前关卡
                const levelParams = getLevelParams(gameState.currentLevel);
                if (gameState.levelScore >= levelParams.winScore) {
                    if (gameState.currentLevel < config.totalLevels) {
                        // 进入下一关
                        setTimeout(() => {
                            gameState.currentLevel++;
                            initGame(gameState.currentLevel);
                            gameStatusElement.textContent = `恭喜通过第 ${gameState.currentLevel - 1} 关！`;
                            gameStatusElement.style.color = "#00ff00";
                        }, 1000);
                    } else {
                        // 完成所有关卡
...
function updateGridAfterMatch() {
    ...
    allTiles.forEach(tile => tile.classList.add('falling'));
...
function updateUI() {
    ...
    if (levelElement) {
        levelElement.textContent = `${gameState.currentLevel}/${config.totalLevels}`;
    }
    ...
    // 更新步数颜色
    if (gameState.movesLeft <= 5) {
        movesElement.style.color = "#ff3366";
    } else if (gameState.movesLeft <= 10) {
        movesElement.style.color = "#ffcc00";
    } else {
        movesElement.style.color = "#00ffff";
    }
}
...
function endGame(isWin) {
    ...
    if (isWin) {
        playSound('win');
        if (gameState.currentLevel >= config.totalLevels) {
            gameStatusElement.textContent = `恭喜通关所有关卡！最终得分：${gameState.score}`;
        } else {
        ...
        gameStatusElement.textContent = `游戏结束！最终得分：${gameState.score}（第 ${gameState.currentLevel} 关）`;
        gameStatusElement.style.color = "#ff3366";
    }
}
