...
class ContraGame {
    constructor() {
        ...
        msg.className = 'god-mode-message';
        msg.textContent = this.godMode ? '无敌模式 ON' : '无敌模式 OFF';
        msg.style.cssText = `
        position: absolute;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
        color: ${this.godMode ? '#0f0' : '#f00'};
        font-size: 24px;
        ...
        // 显示秒杀模式提示
        showOneHitKillMessage() {
        const msg = document.createElement('div');
        msg.className = 'onehit-message';
        msg.textContent = this.oneHitKill ? '秒杀模式 ON' : '秒杀模式 OFF';
        msg.style.cssText = `
        position: absolute;
        top: 60%;
        left: 50%;
        transform: translate(-50%, -50%);
        color: ${this.oneHitKill ? '#ff0' : '#f00'};
        font-size: 24px;
        text-shadow: 2px 2px 0 #000;
        z-index: 200;
        pointer-events: none;
    ...
    bindEvents() {
        ...
        // ===== 新增：为游戏结束界面添加点击 =====
         const gameOverScreen = document.getElementById('game-over-screen');
             if (gameOverScreen) {
            gameOverScreen.addEventListener('click', () => this.handleEnter());
        ...
        const levelClearScreen = document.getElementById('level-clear-screen');
            if (levelClearScreen) {
            levelClearScreen.addEventListener('click', () => this.handleEnter());
            levelClearScreen.addEventListener('touchstart', (e) => {
            e.preventDefault();
            this.handleEnter();
            }, { passive: false });
        }
        ...
        const continueScreen = document.getElementById('continue-screen');
            if (continueScreen) {
            continueScreen.addEventListener('click', () => this.handleEnter());
            continueScreen.addEventListener('touchstart', (e) => {
            e.preventDefault();
            this.handleEnter();
            }, { passive: false });
        }
    ...
    handleKeyDown(e) {
        switch(e.code) {
            ...
            case 'Enter':
                ...
                console.log(`无敌模式: ${this.godMode ? '开启' : '关闭'}`);
                // 可选：在屏幕上显示提示
                this.showGodModeMessage();
                break;
                case 'KeyK':
                // 切换秒杀模式
                this.oneHitKill = !this.oneHitKill;
                console.log(`秒杀模式: ${this.oneHitKill ? '开启' : '关闭'}`);
                this.showOneHitKillMessage();
                break;
                }
    ...
    createBoss() {
        ...
        let bossX = level.length - 350;
        const bossPlatformY = groundY - 80;
        // 在BOSS区域创建一个固定平台
        this.platforms.push({
            x: level.length - 500,
            y: bossPlatformY,
        ...
        let bossY = bossPlatformY - 120; // 不同BOSS可微调
        
        switch(level.bossType) {
            case 'wallFortress':
                this.boss = new WallFortress(bossX, bossY);
                break;
            case 'reactorCore':
                this.boss = new ReactorCore(bossX, bossY);
                break;
            case 'alienBeast':
                this.boss = new AlienBeast(bossX, bossY);
                break;
            case 'hologramSystem':
                this.boss = new HologramSystem(bossX, bossY);
                break;
            case 'hoverCraft':
                this.boss = new HoverCraft(bossX, bossY);
                break;
            case 'giantSoldier':
                this.boss = new GiantSoldier(bossX, bossY);
                break;
            case 'gateKeeper':
                this.boss = new GateKeeper(bossX, bossY);
                break;
            case 'alienHeart':
                this.boss = new AlienHeart(bossX, bossY);
                break;
            default:
                this.boss = new WallFortress(bossX, bossY);
        }
    ...
    spawnEnemy() {
        ...
        if (this.spawnTimer > spawnRate && this.enemies.length < 5) {
            ...
            let spawnX = this.camera.x + GAME_CONFIG.LOGICAL_WIDTH + 50;
            let spawnY = GAME_CONFIG.LOGICAL_HEIGHT - 80 - Math.random() * 100;
    ...
    drawGame() {
        ...
        ctx.textAlign = 'right';
        ctx.fillText('GOD', GAME_CONFIG.LOGICAL_WIDTH - 10, 30);
        }
        ...
        ctx.font = '12px "Press Start 2P"';
        ctx.textAlign = 'right';
        ctx.fillText('1HIT', GAME_CONFIG.LOGICAL_WIDTH - 10, 50);
    }}
...
class Player {
    ...
    update(input, game) {
        ...
        this.jumpKeyPressed = input.jump;
    ...
    // 确保瞄准方向有效（新增代码）
    let dx = input.aimDirection.x;
    let dy = input.aimDirection.y;
    if (dx === 0 && dy === 0) {
        dx = this.direction;   // 默认朝当前面向方向
    }
...
class Bullet {
    ...
    checkPlayerCollision(game) {
        ...
        if (this.x < game.player.x + game.player.width &&
            ...
            // 传入子弹作为伤害来源，触发击退
            game.player.takeDamage(game, this);
            this.active = false;
        }
    }
...
class Boss {
    ...
    const leftBound = game.levelData.length - 500;
    const rightBound = game.levelData.length - 200;
    if (this.x > rightBound) this.moveDirection = -1;
    if (this.x < leftBound) this.moveDirection = 1;
...
class AlienBeast extends Boss {
    ...
    attack(game) {
        ...
        game.bullets.push(new Bullet(
            ...
            dx > 0 ? -8 : 8,  // 向左射击（如果玩家在右边则朝左，否则朝右？这里统一向左更合理）
            -2,                // 略微向上
            'spread',
            false
        ));
