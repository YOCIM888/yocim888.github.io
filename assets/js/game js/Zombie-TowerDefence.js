...
const zombieData = {
    normal: {
        ...
        getSpeed: function() { return Math.random() * 0.2 + 0.3; },
        hp: 150,
        damage: 15,
        attackSpeed: 1200
    },
    conehead: {
        ...
        getSpeed: function() { return Math.random() * 0.15 + 0.25; },
        hp: 350,
        damage: 20,
        attackSpeed: 1200
    },
    fast: {
        ...
        getSpeed: function() { return Math.random() * 0.2 + 0.6; },
        hp: 80,
        damage: 10,
        attackSpeed: 800
    }
...
function getLawnWidth() {
    return document.getElementById('lawn').offsetWidth;
}

function getCellWidth() {
    return getLawnWidth() / 9;
}
...
function showShovelAnimation(row) {
    ...
    const firstCell = gameState.cells.find(c => c.row === row && c.col === 0);
    if (firstCell && firstCell.rect) {
        shovelAnim.style.top = `${firstCell.rect.top + firstCell.rect.height/2 - 15}px`;
    } else {
        shovelAnim.style.top = `${row * getCellWidth() + 35}px`;
    }
...
function placePlant(row, col) {
    ...
    // 修复：使用存储的单元格位置信息
    if (cell.rect) {
        const plantSize = cell.rect.width * 0.7;
        plantEl.style.width = `${plantSize}px`;
        plantEl.style.height = `${plantSize}px`;
        plantEl.style.left = `${cell.rect.left + cell.rect.width/2 - plantSize/2}px`;
        plantEl.style.top = `${cell.rect.top + cell.rect.height/2 - plantSize/2}px`;
    } else {
        const cellWidth = getCellWidth();
        const cellHeight = getCellWidth() * 5 / 9;
        const plantSize = cellWidth * 0.7;
        plantEl.style.width = `${plantSize}px`;
        plantEl.style.height = `${plantSize}px`;
        plantEl.style.left = `${col * cellWidth + cellWidth * 0.15}px`;
        plantEl.style.top = `${row * cellHeight + cellHeight * 0.15}px`;
    }
...
function spawnZombie() {
    ...
    zombieEl.style.left = getLawnWidth() + 'px';

    const cell = gameState.cells.find(c => c.row === row && c.col === 8);
    if (cell && cell.rect) {
        const zombieHeight = cell.rect.height * 0.85;
        const zombieWidth = cell.rect.width * 0.65;
        zombieEl.style.width = `${zombieWidth}px`;
        zombieEl.style.height = `${zombieHeight}px`;
        zombieEl.style.top = `${cell.rect.top + cell.rect.height/2 - zombieHeight/2}px`;
    } else {
        const lawnEl = document.getElementById('lawn');
        const cellHeight = lawnEl ? lawnEl.offsetHeight / 5 : getLawnWidth() / 9;
        const cellWidth = getCellWidth();
        const zombieHeight = cellHeight * 0.85;
        const zombieWidth = cellWidth * 0.65;
        zombieEl.style.width = `${zombieWidth}px`;
        zombieEl.style.height = `${zombieHeight}px`;
        zombieEl.style.top = `${row * cellHeight + cellHeight * 0.075}px`;
    }
...
function gameLoop(timestamp) {
    ...
    const deltaTime = Math.min(rawDelta, 100) / FRAME_DURATION;

    const currentTime = Date.now();
    const lawnWidth = getLawnWidth();
    const cellWidth = getCellWidth();
        ...
        if (plant.ability === 'sunProducer') {
            if (currentTime - plant.lastActionTime > plant.cooldown) {
                ...
                if (plant.cell && plant.cell.rect) {
                    ...
                    sunEffect.style.left = `${plant.col * cellWidth + cellWidth * 0.4}px`;
                    sunEffect.style.top = `${plant.row * cellWidth + cellWidth * 0.4}px`;
                }
            ...
            const zombieInRow = gameState.zombies.find(z =>
                z.row === plant.row &&
                z.element.offsetLeft < lawnWidth
            );
    ...
    gameState.zombies.forEach(zombie => {
        ...
        const currentLeft = parseInt(zombie.element.style.left) || lawnWidth;

        const cellCol = Math.max(0, Math.min(8, Math.floor((currentLeft - 10) / cellWidth)));
        ...
        if (cell && cell.plant) {
            ...
            const newLeft = currentLeft - zombie.speed * deltaTime;
...
function shootPea(plant) {
    ...
    if (plant.cell && plant.cell.rect) {
        ...
        const cellWidth = getCellWidth();
        projectileEl.style.left = `${plant.col * cellWidth + cellWidth * 0.6}px`;
        projectileEl.style.top = `${plant.row * cellWidth + cellWidth * 0.4}px`;
    }
...
window.addEventListener('resize', () => {
    ...
    gameState.plants.forEach(plant => {
        if (plant.cell && plant.cell.rect) {
            const plantSize = plant.cell.rect.width * 0.7;
            plant.element.style.left = `${plant.cell.rect.left + plant.cell.rect.width/2 - plantSize/2}px`;
            plant.element.style.top = `${plant.cell.rect.top + plant.cell.rect.height/2 - plantSize/2}px`;
        }
    });

    gameState.zombies.forEach(zombie => {
        const currentLeft = parseInt(zombie.element.style.left);
        if (currentLeft > lawnWidth) {
            zombie.element.style.left = lawnWidth + 'px';
        }
        const cell = gameState.cells.find(c => c.row === zombie.row && c.col === 8);
        if (cell && cell.rect) {
            const zombieHeight = cell.rect.height * 0.85;
            zombie.element.style.top = `${cell.rect.top + cell.rect.height/2 - zombieHeight/2}px`;
        }
    });

    gameState.projectiles.forEach(projectile => {
        const currentLeft = parseInt(projectile.element.style.left);
        if (currentLeft > lawnWidth + 50) {
            projectile.element.style.left = (lawnWidth + 50) + 'px';
        }
    });

    gameState.suns.forEach(sun => {
        const currentLeft = parseFloat(sun.element.style.left);
        const currentTop = parseFloat(sun.element.style.top);
        if (currentLeft > lawnWidth) {
            sun.element.style.left = (lawnWidth - 30) + 'px';
        }
    });
});
