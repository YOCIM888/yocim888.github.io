...
function setupCanvas() {
    ...
    player.y = BASE_HEIGHT - 120;
}

// 初始化星星背景
function initStars() {
    stars.length = 0;
    const starCount = Math.floor((BASE_WIDTH * BASE_HEIGHT) / 8000);
    ...
    if (gameState.lightningActive && gameState.lightningTargets.length > 0) {
        ...
        gameState.lightningTargets.forEach(target => {
            ...
            while (currentY < targetY) {
                ...
                currentX = Math.max(0, Math.min(BASE_WIDTH, currentX));
...
function useBomb() {
    ...
    // 大爆炸效果
    createExplosion(BASE_WIDTH / 2, BASE_HEIGHT / 2, 150, '#ffff00', '#ff6600');
    createParticles(BASE_WIDTH / 2, BASE_HEIGHT / 2, '#ffff00', 100, 2);
    createParticles(BASE_WIDTH / 2, BASE_HEIGHT / 2, '#ff6600', 100, 2);
...
function spawnEnemy() {
    ...
    enemies.push({
        x: Math.random() * (BASE_WIDTH - config.width),
        y: -config.height,
        width: config.width,
...
function destroyEnemy(enemy, ignoreShield = false) {
    ...
    if (enemy.type === 'boss') {
        ...
        createFloatingText(BASE_WIDTH / 2, BASE_HEIGHT / 2 - 50, 'BOSS击败!', '#ff0000');
    } else {
...
function update() {
    try {
        ...
        if ((keys.ArrowRight || keys.d || keys.D) && player.x < BASE_WIDTH - player.width) player.x += player.speed;
        if ((keys.ArrowUp || keys.w || keys.W) && player.y > 0) player.y -= player.speed;
        if ((keys.ArrowDown || keys.s || keys.S) && player.y < BASE_HEIGHT - player.height) player.y += player.speed;
        ...
        for (let i = bullets.length - 1; i >= 0; i--) {
            ...
            if (bullet.y < -bullet.height || bullet.x < 0 || bullet.x > BASE_WIDTH) {
                bullets.splice(i, 1);
            }
        }
        ...
        for (let i = missiles.length - 1; i >= 0; i--) {
            ...
            // 检查是否超出屏幕
            if (missile.y < -50 || missile.x < -50 || missile.x > BASE_WIDTH + 50) {
                missiles.splice(i, 1);
            }
        }
        ...
        for (let i = enemyBullets.length - 1; i >= 0; i--) {
            ...
            if (bullet.y > BASE_HEIGHT + 20 || bullet.y < -20 || bullet.x < -20 || bullet.x > BASE_WIDTH + 20) {
        ...
        for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (enemy.x > BASE_WIDTH - enemy.width) enemy.x = BASE_WIDTH - enemy.width;
...
function draw() {
    try {
        ...
        // 计算居中偏移
        const offsetX = (canvas.width - BASE_WIDTH * gameScale) / 2;
        const offsetY = (canvas.height - BASE_HEIGHT * gameScale) / 2;
...
// 导出游戏状态和技能函数到全局作用域，供虚拟控制使用
window.gameState = gameState;
window.useMissile = useMissile;
window.useShield = useShield;
window.useLightning = useLightning;
