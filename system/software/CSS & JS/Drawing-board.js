...
class CyberCanvas {
    ...
    drawBrush(x, y) {
        ...
        if (distance > this.state.smoothness) {
            this.ctx.lineTo(x, y);
            this.ctx.stroke();
            this.ctx.beginPath();
            this.ctx.moveTo(x, y);
    ...
    drawLinePreview(x, y) {
        ...
        this.overlayCtx.moveTo(this.state.lastX, this.state.lastY);
    ...
    drawRectPreview(x, y) {
        ...
        this.overlayCtx.strokeStyle = this.state.currentColor;
        this.overlayCtx.lineWidth = this.state.brushSize;
        this.overlayCtx.strokeRect(this.state.lastX, this.state.lastY, width, height);
    }

    drawCirclePreview(x, y) {
        this.overlayCtx.clearRect(0, 0, this.overlay.width, this.overlay.height);
        const radius = Math.sqrt(
            Math.pow(x - this.state.lastX, 2) + 
            Math.pow(y - this.state.lastY, 2)
        );
        
        this.overlayCtx.beginPath();
        this.overlayCtx.arc(this.state.lastX, this.state.lastY, radius, 0, Math.PI * 2);
        this.overlayCtx.strokeStyle = this.state.currentColor;
        this.overlayCtx.lineWidth = this.state.brushSize;
        this.overlayCtx.stroke();
    }
    ...
    drawOverlay(e) {
        ...
        switch(this.state.currentTool) {
            ...
            case 'eraser':
                ...
                this.overlayCtx.fillStyle = this.state.currentTool === 'eraser' 
                    ? 'rgba(255,255,255,0.5)' 
                    : this.state.currentColor + '80';
    ...
    finalizeLine() {
        ...
        this.ctx.lineTo(this.state.lastX, this.state.lastY);
        this.ctx.stroke();
    }

    finalizeRect() {
        this.ctx.strokeRect(this.state.lastX, this.state.lastY, 0, 0);
    }

    finalizeCircle() {
        this.ctx.beginPath();
        this.ctx.arc(this.state.lastX, this.state.lastY, 0, 0, Math.PI * 2);
        this.ctx.stroke();
    }
